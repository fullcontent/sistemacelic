...
/*
|--------------------------------------------------------------------------
| Web Routes - Sistema Celic
|--------------------------------------------------------------------------
|
| Aqui são registradas as rotas web para a aplicação. Estas rotas são
...
// ==========================================
// 1. AUTENTICAÇÃO E PÁGINAS PÚBLICAS
// ==========================================
...
Route::prefix('admin')->group(function () {
	...
	Route::get('/faturamento/pdf/{id}', 'FaturamentoController@printPDF')->name('faturamento.pdf');
	...
	// Cadastro Rápido de Empresas & Unidades
	...
	// Ações específicas de Pendências
	...
	Route::get('/servico/meus/', 'ServicosController@meusServicos')->name('servico.meus');
	...
	Route::get('/historico/{id}/edit', 'ServicosController@editInteracao')->name('interacao.edit');
...
// ==========================================
// 4. NOTIFICAÇÕES E NOTIFICAÇÕES RÁPIDAS
// ==========================================
...
// ==========================================
// 5. APIS E ENDPOINTS GLOBAIS
// ==========================================

Route::prefix('api')->group(function () {

	Route::get('/servico/{id}/data', 'ApiController@getServicoData')->name('api.servico.data');
	Route::get('/unidades/get', 'ApiController@getUnidades');
	Route::get('/empresas/get', 'ApiController@getEmpresas');
	Route::get('/responsaveis/get', 'ApiController@getResponsaveis');
	Route::get('/servicosLpu/get', 'ApiController@getServicosLpu');
	Route::get('/servicosLpu/find', 'ApiController@getServicoLpuById');

	Route::get('/getAllServicesJson', 'ApiController@getAllServicesJSON')->name('getAllServicesJSON');
	Route::get('/getClienteServicesJSON', 'ApiController@getClienteServicesJSON')->name('getClienteServicesJSON');
	Route::get('/getUnidadesByState', 'DashboardController@getUnidadesByState')->name('api.getUnidadesByState');
	Route::get('/getUnidadesByRegion', 'DashboardController@getUnidadesByRegion')->name('api.getUnidadesByRegion');
	Route::get('/getRazaoSocial', 'ApiController@getRazaoSocial')->name('api.getRazaoSocial');
	Route::get('/getPendenciasFromUnidade', 'ApiController@getPendenciasFromUnidade')->name('api.getPendenciasFromUnidade');
	Route::get('/getDadosCastro', 'ApiController@getDadosCastro')->name('api.getDadosCastro');
	Route::get('/saveDadosCastro', 'ApiController@saveDadosCastro')->name('api.saveDadosCastro');

	Route::get('/servico/search', 'ApiController@searchService')->name('api.servico.search');
	Route::get('/getPrestadorInfo', 'ApiController@getPrestadorInfo')->name('api.getPrestadorInfo');
...
Route::get('/test', function () {
	$servicos = App\Models\Servico::with('pendencias')
		...
		->select('id', 'nome', 'os', 'unidade_id', 'tipo', 'protocolo_anexo', 'laudo_anexo', 'solicitante', 'responsavel_id', 'coresponsavel_id', 'licenciamento', 'departamento', 'situacao', 'created_at')
		->whereDoesntHave('financeiro')
		->count();

	return $servicos;

	// NFS-e Routes Duplicadas para Teste (Mantido conforme a regra de não alterar duplicações)
	Route::prefix('nfse')->group(function () {
		Route::get('/', 'NfseController@index')->name('nfse.index');
		Route::get('/configuracoes', 'NfseController@indexConfig')->name('nfse.config');
		Route::post('/configuracoes', 'NfseController@upsertConfig')->name('nfse.config.save');
